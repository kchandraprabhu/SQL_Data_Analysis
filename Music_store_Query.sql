...
SELECT billing_country,COUNT(billing_country) FROM invoice GROUP BY billing_country ORDER BY count desc;
...
SELECT billing_city, SUM(total) AS grand_total FROM invoice GROUP BY billing_city ORDER BY grand_total DESC;
...
SELECT artist.name, COUNT(track.track_id) AS Total_Rock_Tracks
FROM artist
...
FROM track
WHERE milliseconds > (
	SELECT AVG(milliseconds) AS ave_track_length
	FROM track
)
...
/* Total Sales */
SELECT (sum(unit_price) * sum(quantity)) AS Total_Sales FROM invoice_line;
/* Total Albums Purchased By an Country */
SELECT COUNT(album.album_id) AS Album_Sold, customer.country 
FROM album
JOIN track ON album.album_id = track.album_id
